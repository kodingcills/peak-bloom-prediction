...
class TestCompilerGates(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Re-add columns defensively in case compute_bio_thermal_path changes
        self.df_mock['bio_year'] = 2020
    ...
    def test_smart_rescale_tenths(self):
        ...
        # Force summer max < 15C so heuristic triggers
...
class TestAnomalyPerturbation(unittest.TestCase):
    ...
    def test_physical_constraints(self):
        """ T2: TMIN <= TAVG <= TMAX holds after perturbation """
        class ExtremeEngine:
            def get_monthly_anomaly(self, site, year, month, var):
                if var == 'TMIN':
                    return 20.0  # Push TMIN above others
                return 0.0
    ...
    def test_fallback_behavior(self):
        ...
        try:
            ...
            # Fallback disabled -> should hard-fail
            with self.assertRaises(FileNotFoundError):
                sae.AnomalyEngine(allow_fallback=False)

            # Fallback enabled -> should warn and proceed
            with self.assertLogs(level="WARNING") as cm:
                engine = sae.AnomalyEngine(allow_fallback=True)

            # Ensure we actually emitted the expected warning
